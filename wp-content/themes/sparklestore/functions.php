...
function product_query( $q ){
    //Sản phẩm Nổi bật : http://www.domain.com/cua-hang/?is_featured=1
    if(isset($_REQUEST['is_featured'])){
        $is_featured = isset($_REQUEST['is_featured']) ? (int)$_REQUEST['is_featured'] : "";
        if($is_featured>=0){
            ...
            $q->set( 'tax_query', $tax_query ); //echo 1111;exit;
        }
    }
	
	//Sản phẩm Giảm giá : http://www.domain.com/cua-hang/?is_sale=1
    $on_sale = isset($_REQUEST['is_sale']) ? (int)$_REQUEST['is_sale'] : 0;
    if($on_sale){
        $meta_query = $q->get( 'meta_query' );
        $meta_query[] = array(
                            'key' => '_sale_price',
